...
def fix_q27_improved(question_dir: Path, pdf_path: Path, test_name: str) -> Dict[str, Any]:
    ...
    if matches:
        ...
        print("   ✅ Imagen actualizada con bbox mejorado")
...
def fix_q38_alternatives(question_dir: Path, pdf_path: Path, test_name: str) -> Dict[str, Any]:
    ...
    print("   ✅ Imágenes agregadas a las 4 alternativas")
    return {"success": True}
...
def process_q50(question_dir: Path, pdf_path: Path, test_name: str) -> Dict[str, Any]:
    ...
    if not result.get("success") and "Mismatch" in str(result.get("error", "")):
        print("   ⚠️  Procesamiento falló por mismatch, intentando corrección manual...")
        ...
        if processed_file.exists():
            ...
            if api_key:
                ...
                if result.get("success"):
                    print("   ✅ Q50 regenerada exitosamente desde processed_content")
                    return {"success": True}
...
def fix_q65_alternatives(question_dir: Path, pdf_path: Path, test_name: str) -> Dict[str, Any]:
    ...
    if len(matches) == 4:
        ...
        if len(valid_urls) == 4:
            print("   ✅ Las 4 alternativas ya tienen imágenes válidas")
            return {"success": True}

    # Si faltan imágenes, extraerlas del PDF
    print("   📸 Extrayendo imágenes de alternativas del PDF")
