...
def process_pdf(input_pdf_path: str, output_dir: str, start_page_in_original: int = 1, run_summary: list = None, only_part: int = None):
    ...
    part_summary = {"part_name": os.path.basename(input_pdf_path), "status": "Skipped", "total_questions": 0, "reason": ""}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Segments a PDF into questions and references using document-wide processing.")
    ...
    parser.add_argument("--start-part", type=int, default=1, help="Start processing from a specific part number.")
    parser.add_argument("--only-part", type=int, default=None, help="Process only a single specific part.")
