...
def main():
    ...
    if q14_path.exists():
        ...
        if extract_page_from_pdf(str(q14_path), 3, str(q14_path)):
            print(f"   ✅ Q14 corregida (extraída página 3)")
        else:
            print(f"   ❌ Error corrigiendo Q14")
            # Restaurar backup
            shutil.move(backup_path, q14_path)
    else:
        print(f"   ⚠️  Q14.pdf no encontrado")
    ...
    if q56_path.exists():
        ...
        if extract_page_from_pdf(str(q56_path), 3, str(q56_path)):
            print(f"   ✅ Q56 corregida (extraída página 3)")
        else:
            print(f"   ❌ Error corrigiendo Q56")
            # Restaurar backup
            shutil.move(backup_path, q56_path)
    else:
        print(f"   ⚠️  Q56.pdf no encontrado")
