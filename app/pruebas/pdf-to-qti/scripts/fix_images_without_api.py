...
from modules.pdf_processor import render_image_area, _trim_whitespace
...
def fix_q64_special(
    ...
    print(f"✅ Q64 corregida: 2 imágenes separadas por texto intermedio")
    return {"success": True}
...
def fix_image_for_question(
    ...
    if option_labels and len(all_images) > 1 and options_have_images:
        ...
        print(f"📋 Procesando 1 imagen principal")
    ...
    if not skip_additional_adjustments and not was_adjusted and overlapping_blocks:
        print(f"   ⚠️  No se pudo ajustar con bloques superpuestos, intentando enfoque alternativo")
    ...
    try:
        ...
        if len(images_to_process) >= 2 and option_labels:
            print(f"   📊 Múltiples imágenes de opciones detectadas, generando imágenes separadas...")
            ...
            if drawings:
                ...
                if drawing_ys:
                    ...
                    if abs(current_y_center - drawing_y_center) > 100:  # Más de 100px de diferencia
                        ...
                        print(f"   🔧 Usando bbox de dibujos en vez del detectado")
        ...
        print(f"📤 Subiendo imagen corregida a S3...")
        s3_url = upload_image_to_s3(
            image_base64=image_base64,
        ...
        if option_images and len(option_images) > 0:
            ...
            print(f"✅ XML actualizado con imágenes de opciones en cada alternativa")
        else:
            ...
            if len(all_images) > 1 and not option_labels:
                ...
                if body_images:
                    ...
                    if len(body_img_matches) == len(body_images):
                        ...
                        with open(xml_file, "w", encoding="utf-8") as f:
                            f.write(new_xml)
                        print(f"⚠️  Se actualizó XML con imagen única como fallback")
                else:
                    print(f"⚠️  No se pudieron generar las imágenes separadas")
            else:
                ...
                if re.search(pattern, xml_content):
                    ...
                    print(f"✅ XML actualizado con nueva URL S3")
                else:
                    print(f"⚠️  No se encontró patrón de imagen en XML para reemplazar")
