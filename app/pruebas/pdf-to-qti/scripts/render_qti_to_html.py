...
def _render_choices(elem, correct_value: str | None, math_ns: str) -> list[str]:
    ...
    for choice in choices:
        ...
        html_parts.append(f'  <input type="radio" name="response" id="{choice_id}" value="{choice_id}" {checked} />')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Renderizar QTI XML a HTML visual")
    parser.add_argument("--question", type=int, default=7, help="Número de pregunta a renderizar")
    parser.add_argument("--output-dir", default="./output/paes-invierno-2026-new", help="Directorio con los QTI generados")
    parser.add_argument("--output-html", help="Archivo HTML de salida (opcional)")
