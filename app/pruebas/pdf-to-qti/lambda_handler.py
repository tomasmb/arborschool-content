    ...
    try:
        ...
        for field in required_fields:
            if field not in body:
                return {"statusCode": 400, "body": json.dumps({"success": False, "error": f"Missing required field: {field}"})}

        # Validate PDF input (either URL or base64)
        if "pdf_url" not in body and "pdf_base64" not in body:
            return {"statusCode": 400, "body": json.dumps({"success": False, "error": "Either pdf_url or pdf_base64 must be provided"})}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = process_single_question_pdf(pdf_path, output_dir, openai_api_key, validation_endpoint)

            # Upload to S3 if configured
            s3_url = None
            if result.get("success") and "aws_credentials" in body and "s3_config" in body:
                s3_url = upload_to_s3(result["qti_xml"], result["title"], body["aws_credentials"], body["s3_config"])

        processing_time = time.time() - start_time

        # Prepare response
        response_body = {"success": result.get("success", False), "processing_time_seconds": round(processing_time, 2)}
        ...
        return {
            ...
            "body": json.dumps({"success": False, "error": str(e), "error_type": type(e).__name__}),
        }
...
def upload_to_s3(qti_xml: str, title: str, aws_credentials: Dict[str, str], s3_config: Dict[str, str]) -> str:
    ...
    try:
        ...
        # Upload to S3
        s3_client.put_object(Bucket=s3_config["bucket_name"], Key=s3_key, Body=qti_xml.encode("utf-8"), ContentType="application/xml")
