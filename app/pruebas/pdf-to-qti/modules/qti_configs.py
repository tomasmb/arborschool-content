...
def extract_example_xml(examples_xml: str, type: str, index: int = 0) -> str:
    ...
    item_match = re.search(
        r'<qti-assessment-item[\s\S]*?</qti-assessment-item>',
        extracted_xml
    )
    if item_match:
...
inline_choice_config = QuestionConfig(
    ...
    prompt_instructions=INLINE_CHOICE_INSTRUCTIONS
)
...
QTI_TYPE_CONFIGS = {
    c.type: {
        "type": c.type,
        "promptInstructions": c.prompt_instructions.strip(),
        "exampleXml": c.example_xml
    }
