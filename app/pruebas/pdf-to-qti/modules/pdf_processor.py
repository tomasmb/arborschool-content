...
from .image_processing.image_detection import (
...
from .image_processing.multipart_images import detect_multipart_question_images, filter_images_for_multipart_question, detect_part_specific_images_with_ai
...
def extract_images_and_tables(
    ...
    print(f"📊 Step 1: PyMuPDF table detection...")
    ...
    if not detected_tables:
        print(f"📊 Step 1.5: PyMuPDF found no tables, trying reconstruction...")
    ...
    if openai_api_key:
        ...
        if ai_analysis_result.get("success", False):
            ...
            print(f"🧠 ⚠️ AI analysis failed, falling back to PyMuPDF only")
            ai_categories = {}
            has_visual_content = should_use_ai_image_detection(all_blocks, openai_api_key)
    else:
        print(f"🧠 No OpenAI API key, using fallback image detection")
        ai_categories = {}
    ...
    if image_blocks:
        print(f"📸 Step 4a: Processing PyMuPDF image blocks...")
        ...
        if candidate_images and has_visual_content:
            print(f"📸 Step 4b: Assessing PyMuPDF image adequacy...")
            ...
            if adequacy_result["adequate"]:
                ...
                print(f"📸 Step 4c: Using smart expansion of PyMuPDF bbox...")
        ...
        if ai_detected_images:
            ...
            for i, img_area in enumerate(ai_detected_images):
                ...
                if 'ai_categories' in img_area and not ai_categories:
                    ...
                    print(f"📸 ✅ Using AI categories from image detection")
