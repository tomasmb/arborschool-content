...
from app.question_variants.blueprint_repairs import repair_blueprints
...
# ---------------------------------------------------------------------------
# Public pure function -- shared by sync (VariantPlanner) and batch paths
# ---------------------------------------------------------------------------
...
def build_planning_prompt(source: SourceQuestion, n: int) -> str:
    ...
    policies = _resolve_planning_policies(construct_contract)
    min_axes = policies["min_axes"]
...
4. Cada variante debe cambiar al menos {min_axes} eje(s) estructural(es) fuertes, según la familia:
   ...
   Política específica: {policies["axes_policy"]}
...
{policies["family_rules"]}
{policies["shapes_instruction"]}
{policies["hard_constraints_instruction"]}
</reglas>
...
def parse_planning_response(
    ...
    *,
    ...
    """Parse planning LLM response into VariantBlueprints.
        ...
        result.append(
            VariantBlueprint(
                ...
                required_reasoning=str(
                    item.get("required_reasoning", ""),
                ).strip(),
                difficulty_target=str(
                    item.get("difficulty_target", "equal_or_harder"),
                ).strip(),
...
def _resolve_planning_policies(
    construct_contract: dict[str, Any],
) -> dict[str, Any]:
    """Compute family rules, axes policy, shapes, and constraint strings."""
    family_rules = "\n".join(
        f"{i}. {r}"
    ...
    axes_policy = (
        "En esta familia intrínsecamente rutinaria, basta con "
        "materializar 1 eje estructural fuerte si la variante no queda "
        "superficial, mejora o preserva distractores plausibles y no es "
        "solo cambio de números."
        if min_axes == 1
    ...
    if allowed_shapes:
        stxt = json.dumps(allowed_shapes, ensure_ascii=False, indent=2)
        shapes_instruction = (
            "POLÍTICA DE SHAPES: Debes seleccionar una 'Variant Shape' "
            f"permitida para cada variante de este catálogo:\n{stxt}\n"
            "Cada variante DEBE tener un 'shape_id' distinto si generas "
            "múltiples variantes. La 'scenario_description' de tu "
            "blueprint DEBE explicar cómo materializarás la Shape elegida "
            "y qué cambios obligatorios cubrirá."
        )
    else:
        shapes_instruction = (
            "No hay Variant Shapes específicas para esta familia. "
            "Utiliza el estándar de la familia."
        )
    hard_constraints = construct_contract.get("hard_constraints", []) or []
    if "must_not_add_auxiliary_transformations" in hard_constraints:
        auxiliary_policy = (
            "No aumentes la cantidad de transformaciones auxiliares "
            "respecto de la fuente: no propongas conversiones, "
            "equivalencias ni pasos intermedios adicionales."
        )
    elif expectation == "low":
        auxiliary_policy = (
            "Evita agregar transformaciones auxiliares gratuitas. Se "
            "permite, como máximo, un paso intermedio breve si ayuda a "
            "des-mecanizar la variante sin cambiar el constructo ni "
            "volverla de una banda de dificultad claramente distinta."
        )
    else:
        auxiliary_policy = (
            "Evita agregar transformaciones auxiliares gratuitas. Solo se "
            "permite un paso intermedio corto y conceptualmente justificado "
            "si mejora la no-mecanizabilidad sin alterar el método central."
        )
    hard_constraints_instruction = (
        "RESTRICCIONES DURAS DEL CONTRATO: no puedes proponer blueprints "
    ...
    return {
        ...
        "shapes_instruction": shapes_instruction,
        "auxiliary_policy": auxiliary_policy,
        "hard_constraints_instruction": hard_constraints_instruction,
    }


# ---------------------------------------------------------------------------
# VariantPlanner class -- sync path (used by --no-batch mode)
# ---------------------------------------------------------------------------


class VariantPlanner:
    """Generates structured blueprints before QTI generation (sync mode)."""
    ...
    def plan_variants(
        ...
        try:
            ...
            plans = parse_planning_response(
                response, construct_contract=construct_contract,
            )
        ...
        return repair_blueprints(plans[:n], construct_contract)
    ...
    def _fallback_plans(self, source: SourceQuestion, n: int) -> List[VariantBlueprint]:
        ...
        return repair_blueprints(plans, contract)
