...
## 2. Inside `app/` (invariants)

Current structure:

- `app/temarios/`  
  - `parsing.py`: single-responsibility module that parses DEMRE temario PDFs and builds structured JSON.

- `app/data/temarios/`  
  - `pdf/`: official DEMRE temario PDFs (source-of-truth inputs).  
  - `json/`: structured JSON versions of each temario, generated by `app/temarios/parsing.py`.

- `app/gemini_client.py`  
  - Reusable Gemini client wrapper (currently using `gemini-2.5-flash`).  
  - Reads `GEMINI_API_KEY` from the project-level `.env` file and applies the\n+    prompting guidelines in `docs/gemini-3-pro-prompt-engineering-best-practices.md`.
...
## 5. Adding a new module under `app/` (staff-level checklist)
...
3. **Expose clean entry points**
   ...
   - Script entry points must be **thin wrappers** that call well-typed functions, not large blobs of logic.

4. **Update docs if needed**
   - If a new pattern or process emerges (e.g. new way of building
     standards JSON), add or update a short section in `docs/`.
   - When you change a public contract (e.g. shape of structured JSON, location of canonical standards),
     update both:
     - `docs/python-best-practices.md` (if you changed a cross-cutting rule), and
     - the most relevant domain doc under `docs/`.
